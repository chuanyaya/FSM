...
TGT_LABELED_CSV = os.path.join(PROJECT_ROOT, 'data/csv/EndoScene_SSDA_10p/train_labeled.csv')
TGT_UNLABELED_CSV = os.path.join(PROJECT_ROOT, 'data/csv/EndoScene_SSDA_10p/train_unlabeled.csv')
...
EXP_NAME = 'meanteacher_fsm_freqperturb_resnet18_10p_fixed'
...
class MixedBatchDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df, transform, domain_label, img_size=256, normalize=True, has_mask=True):
    ...
    def __getitem__(self, idx):
        ...
        result = {'image_weak': torch.from_numpy(image_weak.transpose(2, 0, 1)), 'image_strong': torch.from_numpy(image_strong.transpose(2, 0, 1)), 'domain_label': torch.tensor(self.domain_label, dtype=torch.long)}
        if self.has_mask:
...
def train_one_epoch(model, src_loader, tgt_labeled_loader, tgt_unlabeled_loader, optimizer, loss_fn, epoch, device, writer):
    ...
    epoch_losses = {'total': [], 'sup_src': [], 'sup_tgt': [], 'pseudo': [], 'mask_ratio': [], 'mixed': [], 'freq_perturb': []}
    ...
    for batch_idx in pbar:
        ...
        src_domain = src_batch['domain_label'].to(device)
        ...
        tgt_l_domain = tgt_l_batch['domain_label'].to(device)
        tgt_u_weak = tgt_u_batch['image_weak'].to(device)
        tgt_u_strong = tgt_u_batch['image_strong'].to(device)
        tgt_u_domain = tgt_u_batch['domain_label'].to(device)
        batch_size = src_strong.shape[0]
        labeled_strong = torch.cat([src_strong, tgt_l_strong], dim=0)
        labeled_domain = torch.cat([src_domain, tgt_l_domain], dim=0)
        pred_labeled_original, pred_labeled_mixed = model.student_forward(labeled_strong, labeled_domain, return_both=True)
        pred_src_original = pred_labeled_original[:batch_size]
        pred_tgt_l_original = pred_labeled_original[batch_size:]
        pred_src_mixed = pred_labeled_mixed[:batch_size]
        pred_tgt_l_mixed = pred_labeled_mixed[batch_size:]
        pred_tgt_u_student = model.student_forward(tgt_u_strong, tgt_u_domain, return_both=False)
        if USE_FREQ_PERTURB:
        ...
        original_loss, loss_info = loss_fn(pred_src_original, src_mask, pred_tgt_l_original, tgt_l_mask, pred_tgt_u_student, pred_tgt_u_teacher, epoch=epoch)
        loss_src_mixed = loss_fn.sup_loss_fn(pred_src_mixed, src_mask)
        loss_tgt_l_mixed = loss_fn.sup_loss_fn(pred_tgt_l_mixed, tgt_l_mask)
        mixed_loss = (loss_src_mixed + loss_tgt_l_mixed) / 2
        if USE_FREQ_PERTURB and pred_tgt_u_perturbed is not None:
            freq_perturb_loss = nn.functional.mse_loss(torch.sigmoid(pred_tgt_u_perturbed), torch.sigmoid(pred_tgt_u_student.detach()))
        else:
            freq_perturb_loss = torch.tensor(0.0).to(device)
        total_loss = original_loss + FSM_LOSS_WEIGHT * mixed_loss + FREQ_PERTURB_WEIGHT * freq_perturb_loss
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
        ...
        epoch_losses['mixed'].append(mixed_loss.item())
        epoch_losses['freq_perturb'].append(freq_perturb_loss.item())
        pbar.set_postfix({'loss': f'{total_loss.item():.3f}', 'mixed': f'{mixed_loss.item():.3f}', 'freq': f'{freq_perturb_loss.item():.3f}', 'mask%': f"{loss_info.get('pseudo_mask_ratio', 0) * 100:.0f}"})
        global_step = epoch * num_batches + batch_idx
        if batch_idx % 50 == 0:
            writer.add_scalar('Loss/total', total_loss.item(), global_step)
            writer.add_scalar('Loss/mixed', mixed_loss.item(), global_step)
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, default='best_model.pth', help='Checkpoint filename for test mode')
    args = parser.parse_args()
    if args.mode == 'test':
        test_with_args(args)
        return
    print('=' * 60)
    print('Mean Teacher + FixMatch + FSM (Single GPU)')
    print('=' * 60)
    device = torch.device(f'cuda:{args.gpu}' if torch.cuda.is_available() else 'cpu')
    print(f'Device: {device}')
    print(f'Batch size: {BATCH_SIZE}, LR: {LR}')
    print(f'Confidence threshold: {CONFIDENCE_THRESHOLD}')
    print(f'FSM layers: {FSM_LAYERS}, p={FSM_P}, alpha={FSM_ALPHA}')
    ...
    src_dataset = MixedBatchDataset(src_df, dual_aug, domain_label=0, img_size=IMG_SIZE, has_mask=True)
    tgt_l_dataset = MixedBatchDataset(tgt_l_df, dual_aug, domain_label=1, img_size=IMG_SIZE, has_mask=True)
    tgt_u_dataset = MixedBatchDataset(tgt_u_df, dual_aug, domain_label=2, img_size=IMG_SIZE, has_mask=False)
    ...
    print('\n初始化模型（ResNet18 + FSM + 频率扰动）...')
    print(f'- Backbone: ResNet18')
    print(f'- FSM: layers={FSM_LAYERS}, weight={FSM_LOSS_WEIGHT}')
    print(f'- Frequency Perturbation: eps={FREQ_PERTURB_EPS}, weight={FREQ_PERTURB_WEIGHT}')
    model = MeanTeacherWithFSM(backbone='resnet18', num_classes=1, ema_decay=EMA_DECAY, pretrained=False, fsm_layers=FSM_LAYERS, fsm_p=FSM_P, fsm_alpha=FSM_ALPHA, adaptive_fsm=ADAPTIVE_FSM, use_freq_perturb=USE_FREQ_PERTURB, freq_perturb_eps=FREQ_PERTURB_EPS).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_metrics['dice'] > best_dice:
            ...
            torch.save({'epoch': epoch, 'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'best_dice': best_dice, 'config': {'lr': LR, 'batch_size': BATCH_SIZE, 'confidence_threshold': CONFIDENCE_THRESHOLD, 'fsm_layers': FSM_LAYERS, 'fsm_p': FSM_P, 'fsm_alpha': FSM_ALPHA}}, os.path.join(SAVE_DIR, 'best_model_false_10.pth'))
