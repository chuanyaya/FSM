...
MIXSTYLE_P = 1
MIXSTYLE_ALPHA = 0.1
ADAPTIVE_MIXSTYLE = True
MIXSTYLE_LOSS_WEIGHT = 1
USE_FREQ_PERTURB = True
FREQ_PERTURB_EPS = 0.2
FREQ_PERTURB_WEIGHT = 0.3
EXP_NAME = 'resnet18_mixstyle_kits_v7_freqperturb_fixed'
...
class MixedBatchDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df, transform, domain_label, img_size=256, normalize=True, has_mask=True, stylize=None):
...
def train_one_epoch(model, src_loader, tgt_labeled_loader, tgt_unlabeled_loader, optimizer, loss_fn, epoch, device, writer):
    ...
    for batch_idx in pbar:
        ...
        model.set_mixstyle_active(False)
        ...
        model.set_mixstyle_active(True)
        with torch.no_grad():
        ...
        if USE_FREQ_PERTURB and pred_tgt_u_perturbed is not None:
            diff = torch.abs(pred_tgt_u_perturbed - pred_tgt_u_student).mean().item()
            freq_perturb_loss = nn.functional.mse_loss(torch.sigmoid(pred_tgt_u_perturbed), torch.sigmoid(pred_tgt_u_student.detach()))
            if batch_idx == 0:
                print(f'[DEBUG] FP diff: {diff:.6f}, freq_loss: {freq_perturb_loss.item():.6f}')
        else:
            freq_perturb_loss = torch.tensor(0.0).to(device)
        total_loss = original_loss + MIXSTYLE_LOSS_WEIGHT * mixed_loss + FREQ_PERTURB_WEIGHT * freq_perturb_loss
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
        ...
        pbar.set_postfix({'loss': f'{total_loss.item():.3f}', 'mixed': f'{mixed_loss.item():.3f}', 'freq': f'{freq_perturb_loss.item():.4f}', 'consist': f"{loss_info.get('loss_pseudo', 0):.3f}"})
...
def main():
    ...
    src_stylize = 'bright_warm' if args.domain_shift == 'v7' else None
    tgt_stylize = 'dark_cool' if args.domain_shift == 'v7' else None
    print('=' * 60)
    print(f'ResNet18 + MixStyle v7 (Single GPU, {labeled_ratio} labeled)')
    if args.domain_shift == 'v7':
        print(f'Domain Shift v7: Source=BRIGHT_WARM, Target=DARK_COOL')
    print('=' * 60)
    device = torch.device(f'cuda:{args.gpu}' if torch.cuda.is_available() else 'cpu')
    print(f'Device: {device}')
    print(f'Batch size: {BATCH_SIZE}, LR: {LR}')
    print(f'Loss: Mean Teacher (MSE consistency, unsup_weight={UNSUP_WEIGHT}, rampup={RAMPUP_EPOCHS})')
    print(f'MixStyle layers: {MIXSTYLE_LAYERS}, p={MIXSTYLE_P}, alpha={MIXSTYLE_ALPHA}, weight={MIXSTYLE_LOSS_WEIGHT}')
    ...
    print('=' * 60)
    src_dataset = MixedBatchDataset(src_df, dual_aug, domain_label=0, img_size=IMG_SIZE, has_mask=True, stylize=None)
    tgt_l_dataset = MixedBatchDataset(tgt_l_df, dual_aug, domain_label=1, img_size=IMG_SIZE, has_mask=True, stylize=None)
    tgt_u_dataset = MixedBatchDataset(tgt_u_df, dual_aug, domain_label=2, img_size=IMG_SIZE, has_mask=False, stylize=None)
    ...
    model = MeanTeacherWithMixStyle(backbone='resnet18', num_classes=NUM_CLASSES, ema_decay=EMA_DECAY, pretrained=False, mixstyle_layers=MIXSTYLE_LAYERS, mixstyle_p=MIXSTYLE_P, mixstyle_alpha=MIXSTYLE_ALPHA, adaptive_mixstyle=ADAPTIVE_MIXSTYLE, use_freq_perturb=USE_FREQ_PERTURB, freq_perturb_eps=FREQ_PERTURB_EPS).to(device)
