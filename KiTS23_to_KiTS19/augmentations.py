...
def apply_light_stylization(image):
    transform = A.Compose([A.RandomBrightnessContrast(brightness_limit=0.15, contrast_limit=0.2, p=0.8), A.RandomGamma(gamma_limit=(90, 110), p=0.5), A.GaussNoise(var_limit=(10.0, 30.0), mean=0, p=0.6), A.OneOf([A.GaussianBlur(blur_limit=(3, 3), p=1.0), A.MedianBlur(blur_limit=3, p=1.0)], p=0.3)])
    augmented = transform(image=image)
    return augmented['image']

def apply_strong_stylization(image):
    transform = A.Compose([A.RandomBrightnessContrast(brightness_limit=0.3, contrast_limit=0.4, p=0.9), A.RandomGamma(gamma_limit=(70, 130), p=0.7), A.GaussNoise(var_limit=(30.0, 50.0), mean=0, p=0.8), A.OneOf([A.GaussianBlur(blur_limit=(3, 5), p=1.0), A.MedianBlur(blur_limit=5, p=1.0)], p=0.5), A.HueSaturationValue(hue_shift_limit=20, sat_shift_limit=30, val_shift_limit=20, p=0.5)])
    augmented = transform(image=image)
    return augmented['image']

def apply_extreme_stylization(image):
    transform = A.Compose([A.RandomBrightnessContrast(brightness_limit=0.5, contrast_limit=0.6, p=1.0), A.RandomGamma(gamma_limit=(50, 150), p=0.9), A.GaussNoise(var_limit=(50.0, 80.0), mean=0, p=0.9), A.OneOf([A.GaussianBlur(blur_limit=(5, 9), p=1.0), A.MotionBlur(blur_limit=9, p=1.0), A.MedianBlur(blur_limit=7, p=1.0)], p=0.8), A.ImageCompression(quality_lower=40, quality_upper=60, p=0.7), A.HueSaturationValue(hue_shift_limit=30, sat_shift_limit=50, val_shift_limit=30, p=0.9), A.CLAHE(clip_limit=6.0, tile_grid_size=(8, 8), p=0.7)])
    augmented = transform(image=image)
    return augmented['image']

def apply_bright_warm_stylization(image):
    transform = A.Compose([A.RandomBrightnessContrast(brightness_limit=(0.25, 0.35), contrast_limit=(0.35, 0.45), p=1.0), A.RandomGamma(gamma_limit=(120, 140), p=0.8), A.HueSaturationValue(hue_shift_limit=(-10, -5), sat_shift_limit=(10, 20), val_shift_limit=(10, 20), p=0.9)])
    augmented = transform(image=image)
    return augmented['image']

def apply_dark_cool_stylization(image):
    transform = A.Compose([A.RandomBrightnessContrast(brightness_limit=(-0.2, -0.1), contrast_limit=(-0.3, -0.2), p=1.0), A.RandomGamma(gamma_limit=(75, 90), p=0.8), A.HueSaturationValue(hue_shift_limit=(5, 10), sat_shift_limit=(-15, -5), val_shift_limit=(-15, -5), p=0.9)])
    augmented = transform(image=image)
    return augmented['image']
