...
class AdaptiveMixStyle(nn.Module):
    ...
    def __init__(self, p_start=0.3, p_end=0.1, alpha=0.1, eps=1e-06, mix='crossdomain'):
        ...
        self._activated = True
        self.mixstyle = MixStyle(p=p_start, alpha=alpha, eps=eps, mix=mix)
    ...
    def set_progress(self, progress):
        ...
        self.mixstyle.set_probability(self._current_p)

    def set_activation(self, activated):
        self._activated = activated
        self.mixstyle.set_activation(activated)

    def forward(self, x, domain_labels=None):
        if not self._activated:
            return x
        return self.mixstyle(x, domain_labels)
...
class StyleAugmentor(nn.Module):
    ...
    def __init__(self, method='mixstyle', p=0.2, alpha=0.1, adaptive=True, p_start=0.3, p_end=0.1):
        super().__init__()
        self.method = method
        self.adaptive = adaptive
        if adaptive:
            self.mixstyle = AdaptiveMixStyle(p_start, p_end, alpha, mix='crossdomain')
            self.crossnorm = None
        else:
            self.mixstyle = MixStyle(p=p, alpha=alpha, mix='crossdomain')
            self.crossnorm = CrossNorm(p=p)

    def set_progress(self, progress):
        if self.adaptive and hasattr(self.mixstyle, 'set_progress'):
            self.mixstyle.set_progress(progress)
    ...
    def forward(self, x, domain_labels=None):
        if self.method == 'mixstyle':
            ...
            x = self.mixstyle(x, domain_labels)
...
if __name__ == '__main__':
    ...
    mixstyle = MixStyle(p=1.0, alpha=0.1, mix='crossdomain')
    mixstyle.train()
    out = mixstyle(x, domain_labels)
    print(f'MixStyle输出形状: {out.shape}')
    crossnorm = CrossNorm(p=1.0)
    crossnorm.train()
    out = crossnorm(x, domain_labels)
    print(f'CrossNorm输出形状: {out.shape}')
    adaptive = AdaptiveMixStyle(p_start=0.3, p_end=0.1)
    adaptive.train()
